...
public class ReportService {
    ...
    public List<ServiceRiskDTO> getTopCriticalServices() {
        ...
        for (Dependency d : allDeps) {
            Long from = d.getDependsOn().getId(); // source
            Long to   = d.getService().getId();   // destination
        ...
        for (ServiceEntity svc : allServices) {
            ...
            // Plus il y a de services qui dépendent de lui → plus risqué
            double depScore = totalServices > 0
                    ? (depCount / (double) totalServices) * 40
                    : 0;
            ...
            dto.setStatus(svc.getStatus() != null
                    ? svc.getStatus().toString() : "UNKNOWN");
    ...
    public List<BlastRadiusDTO> getBlastRadiusReport() {
        ...
        for (ServiceEntity svc : allServices) {
            ...
            double score = maxPossible > 0
                    ? Math.min((visited.size() / (double) totalServices) * 100, 100)
                    : 0.0;
